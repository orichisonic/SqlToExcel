...
namespace SqlExport
{
    public class DbToCsv:IDbToCsv
    {
        public  void WriteHeader(IDataReader reader, StreamWriter sw)
        {
        ...
        public  void WriteContent(IDataReader reader, StreamWriter sw)
        {
            for (int i = 0; i < reader.FieldCount; i++)
            {
                if (i > 0)
        ...
        public  int SaveCsv(IDataReader reader, string filename)
        {
        ...
        public  async Task<int> ExportCsvAsync(IDataReader reader, string filename)
        {
            return await Task.Run( () => { return SaveCsv( reader,  filename);} );
        }

        public  StreamWriter GetStreamWriter(string filename, int outCount)
        {
        ...
        public  int SaveCsv(IDataReader reader, string filename, int pagesize)
        {
        ...
        public int ExportCsvAsync(IDataReader reader, string filename, int pagesize)
        {
            return SaveCsv(reader, filename,pagesize);
        }
